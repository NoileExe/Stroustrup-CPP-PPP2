...
public:
	...
	ovector() : elem{nullptr}, sz{0}, space{0}		{  }
	
	ovector(size_t n) : elem{new T*[n]}, sz{0}, space{n}
		{ for(size_t i =0; i < n; ++i)		elem[i] = nullptr; }
	
	explicit ovector(initializer_list<T> lst) : elem{new T*[lst.size()]}, sz{0}, space{lst.size()}
	{
		//cout << "ovector(initializer_list<T> lst)" << endl;
	...
	ovector(const ovector& v) : elem{nullptr}, sz{0}, space{0}
	{
		reserve(v.sz);		//Изменяет все члены класса
		for (int i = 0; i < v.sz; ++i)		elem[i] = new T { v[i] }; //Копируем значения
		sz = v.sz;
	}
	ovector& operator=(const ovector& v);	// Копирующее присваивание
	
	//Перемещающие конструктор и присваивание
	ovector(ovector&& v) : elem{nullptr}, sz{0}, space{0}
	{
		reserve(v.sz);		//Изменяет все члены класса
		for (int i = 0; i < v.sz; ++i)		elem[i] = v.elem[i]; //Копируем указатели
		sz = v.sz;
		v.sz = v.space = 0;		v.elem = nullptr;
	}
	ovector& operator=(ovector&& v);
	...
	T& at(size_t n) {
		if ( n>=sz )		throw out_of_range("ovector range error with index [" + to_string(n) + "]");
		return *elem[n];
	}
	
	const T& at(size_t n) const {
		if ( n>=sz )		throw out_of_range("const ovector range error with index [" + to_string(n) + "]");
		return *elem[n];
	}
...
template<typename T>
ovector<T>& ovector<T>::operator=(const ovector<T>& v)
{
	if (this == &v)		return *this;	//Проверка на самоприсваивание
	//cout << "\nCalled ovector<T>::operator=(const ovector<T>& v)\n";
	
	ovector<T> b (v.sz);			//Выделение новой памяти
...
template<typename T>
ovector<T>& ovector<T>::operator=(ovector<T>&& v)
{
	//cout << "\nCalled ovector<T>::operator=(ovector<T>&& v)\n";
	this->~ovector();
...
template<typename T>
void ovector<T>::resize(size_t newsize, T val)
{
	reserve(newsize);
...
template<typename T>
void ovector<T>::push_back(const T& val)
{
...
template<class T>
void ovector<T>::reserve(size_t newalloc)
{
	if(newalloc <= space)	return;		// Размер никогда не уменьшается
	
	ovector<T> b (newalloc);			//Выделение новой памяти
