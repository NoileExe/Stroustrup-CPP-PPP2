...
public:
	...
	//Увеличение
	iterator insert(iterator p, const T& v);		//Вставить v в вектор после элемента, на который указывает итератор p
...
//Вставить v в вектор после элемента, на который указывает итератор p
//Возвращает итератор вновь добавленного элемента
template<typename T, typename A>
typename vctr<T,A>::iterator vctr<T,A>::insert(iterator p, const T& v)
{
	if( !(&p) )			error("nullptr iterator [vctr insert()]");
	
	auto next = p;
	++next;
	if(next == end())		{ push_back(v);		return end(); }
	
	//Смещение всех элементов после p вправо
	push_back(elem[sz-1]);	//Смещаем последнее значение вправо (кол-во элементов увеличивается на 1)
	for(T* i = &elem[sz-2];   i-1 > &p;   --i)		*i = *(i-1);
...
void vctr<T,A>::push_back(const T& val)
{
	...
	elem = iterator(&elem, sz+1);		//Кол-во элементов увеличивается на 1
