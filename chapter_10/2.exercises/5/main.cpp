	...
	for (int i = 0; i < 12; ++i)
		if ( y.month[i].month != not_a_month ) {
			...
			for (int j = 1; j < 32; ++j) {
				...
				for (int k = 0; k < 24; ++k)
					if ( y.month[i].day[j].hour[k] != not_a_reading )
						ost << "\t\t\t" << k << "  " << y.month[i].day[j].hour[k] << '\n';		// час и температура
			}
		}
	
	
	
	/*ost << " YEAR " << "   MONTH   " << " DAY " << " HOUR " << " TEMP" << '\n';
	
	
	//y.month.size()
	//	y.month[i].day.size()
	//		y.month[i].day[j].hour.size()
	
	
	for (int i = 0; i < 12; ++i)
		if ( y.month[i].month != not_a_month ) {
			//--------------------------------------ВЫВОД ГОДА--------------------------------------
			if ( 999 < y.year  &&  y.year < 10000 ) // (999 ; 10000)
				ost << " " << y.year << " ";
			else if ( 99 < y.year  &&  y.year < 1000 ) // (99 ; 1000)
				ost << "  " << y.year << " ";
			else if ( 9 < y.year  &&  y.year < 100 ) // (9 ; 100)
				ost << "  " << y.year << "  ";
			else if ( -1 < y.year  &&  y.year < 10 ) // (-1 ; 10)
				ost << "   " << y.year << "  ";
			else	error("very large/small year");
			//--------------------------------------ВЫВОД ГОДА--------------------------------------
			
			//-------------------------------------ВЫВОД МЕСЯЦА-------------------------------------
			int space_count = string("   MONTH   ").size() / int_to_month(i).size();
			if ( int_to_month(i).size() < 8 )
				if ( int_to_month(i).size() % 2 == 0 )
					++space_count;

			if ( int_to_month(i).size() < 9 )
				++space_count;
			
			for (int s = 0; s < space_count; ++s)	ost << ' ';
			
			ost << int_to_month(i);
			
			if ( int_to_month(i).size() % 2 == 0 )
				for (int s = 0; s < space_count-1; ++s)		ost << ' ';
			else
				for (int s = 0; s < space_count; ++s)		ost << ' ';
			//-------------------------------------ВЫВОД МЕСЯЦА-------------------------------------
