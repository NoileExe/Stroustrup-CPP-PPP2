...
//Работает только с римскими числами от 1 до 3999 (в том числе результат)
...
Roman_int post_oper(Roman_int); //вызывается из pow_proc(), factorial(), primary()
Roman_int primary(Token_stream&); //вызывается из pow_proc (возведение в степень)
Roman_int expression(Token_stream&); //объявляем, т.к. ф-ция primary() может вызвать expression()

//------------------------------------------------------------------------------

Roman_int pow_proc(Roman_int num)
//возведение входящего числа в степень, возвращает таковых р-тат вычислений
//либо вызывает исключение  
{
	Roman_int pow_num { primary(ts).as_int() };
	...
	*/
	...
	Roman_int res;
	for (int i = 0; i < pow_num.as_int(); ++i)
		res = res*num;
	
	//pow_num = pow(num.as_int(), pow_num.as_int());
	
	/*if ( isinf(pow_num) )
		error ("при возведении в степень число выходит за диапазон DOUBLE ( pow_proc() )");*/
	
	return post_oper(res); //проверка правильности выражения
...
Roman_int factorial(Roman_int num) //Вычисление факториала. На вход только целые положит. числа
{
	/*if (num - int(num) != 0)
...
Roman_int post_oper(Roman_int left)
...
Roman_int primary(Token_stream& ts)
{
	...
	switch (t.kind)
	{
		...
		case SQRT:
		{
			...
			Roman_int d = expression(ts); //получаем конечное число в скобках
			if (d.as_int() < 1)
			...
			return post_oper( sqrt(d.as_int()) );
		}
		...
		case VARIABLE: {	//символ 'a' обозначает что происходит обращение к переменной
			...
			if (t.kind == '=') {//Если изменение значения переменной
				Roman_int d = expression(ts); //получаем новое значение
...
{
	Roman_int left = primary(ts);	//считываем и вычисляем первичное_выражение
	...
	while (true) {
		...
		{
			case '*':
				left = left*primary(ts);	//вычисляем первичное_выражение и умножаем
				break;
			case '/':
			{
				left = left/primary(ts);	//вычисляем первичное_выражение и делим
				break;
			}
			case '%':
			{
				left = left%primary(ts);	//остаток от деления
				break;
			}
...
{
	Roman_int left = term(ts);	//считываем и вычисляем Term
	...
	while (true) {
		...
		{
			case '+':
				left = left+term(ts);	//вычисляем Term и прибавляем
				break;
			case '-':
				left = left-term(ts);	//вычисляем Term и отнимаем
				break;
...
Roman_int declaration(bool cnst)
...
void calculate() //Цикл вычисления выражения
{
	...
	try 
	{
		...
		if ( str == HELPKEY )		cerr << HELP_INSTR;		//ВЫВОД ИНСТРУКЦИИ
		
		else if ( str == TABLEOUTKEY )	var_table.table_out();	//Вывод всех переменных/констант
