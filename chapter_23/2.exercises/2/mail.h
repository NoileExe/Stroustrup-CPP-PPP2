...
typedef multimap<string, Message>::const_iterator Mess_iter;

//Хранит все строки из файла и упрощает доступ к сообщениям
struct Mail_file {
	string name;			//Имя файла
	vector<string> lines;	//Строки по порядку
	multimap<string, Message> sm;	//Сообщения c темой в порядке следования в файле
	...
	Mess_iter begin() const { return sm.begin(); }
	Mess_iter end()   const { return sm.end(); }
