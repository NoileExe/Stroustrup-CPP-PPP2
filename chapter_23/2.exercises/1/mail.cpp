...
//На вход указатель на объект письма и искомая строка (заголовок поля)
	...
	//Для простоты установленно, что каждое сообщение заканчивается строкой "----"
{
	...
	while(p != lines.end())
	{
		...
		if(p != lines.end()  &&  *p == "----") {
			...
			if( has_repeat_field(&m[m.size()-1]) ) {
				ostringstream oss;
				for(const auto& x : m[m.size()-1])		oss << x << endl;
			...
			string addrs = find_field(&m[m.size()-1], "To: ");
	...
	//cout << "\nm.size() & msg_count\t" << m.size() << '\t' << msg_count << "\n\n\n";
	...
	for(const auto& x : m)		++subjects[find_field(&x, "Subject: ")];
	
	for(const auto& x : subjects)
		//Размер строки (темы письма) больше 0 (т.к. find_field может вернуть пустую строку)
		//				и  количество писем с такой темой больше 1-го
		//						и  найдены повторы в получателях писем с одной и той же темой
		if( x.first.size()  &&  x.second > 1  &&  has_repeats_in_string( search_recipient(x.first) ) )
			error("repeat recipients of emails with the same subject (" + x.first + ") [Mail_file() constructor]");
}
...
string Mail_file::search_recipient(const string& s) const
{
	...
	for(const auto& x : m)
		if( find_field(&x, "Subject: ") == s )
		{
			string str = find_field(&x, "To: ");
			if( str.size() )
