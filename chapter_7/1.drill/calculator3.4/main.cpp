...
Token Token_stream::get()
{
	...
	if (cin.eof())   error ("CTRL+Z"); //Закрыть программу? Да или Нет (yes / no)
...
//на вход: имя переменной
{
	...
	error ("попытка получить значение необъявленной переменной ( get_value() )");
}
...
//на вход: имя переменной и её новое значение
{
	for (int i = 0; i < vector_names.size(); ++i)
		if ( vector_names[i].constant && vector_names[i].name == s )
			error ("попытка изменить константу ( set_value() )");
	...
	error ("попытка установить значение для необъявленной переменной ( set_value() )");
}
	...
	switch (t.kind)
	{
		...
		case VARIABLE: { //символ 'a' обозначает что происходит обращение к переменной
			...
			if (t.kind == '=') {//Если изменение значения переменной
				...
				ts.putback(t);
				return post_oper(get_value(strname));
			}
		}
...
try
{
	cout << "Добро пожаловать в программу калькулятор 2020 по Бьярне Страуструпу.\n\n"
