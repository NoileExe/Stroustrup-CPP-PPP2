...
namespace Graph_lib {
	...
	ostream& operator<<(ostream& ost, const Font& f) {
		string str;
		switch (f.as_int()) {
	...
	Bar_chart::Bar_chart (Point o, const vector<double>& d, double ln,   double xs,  double ys)
						: orig{o},                         xlenght{ln}, xscale{xs},  yscale{ys}
	{
		for (double e : d)
			vals.push_back( make_pair(std::nan("1"), e) );	//Значения по X указываются как "не число"
		
		chart_points();
	}
	
	Bar_chart::Bar_chart (Point o, const vector<double>& x, const vector<double>& y, double ln,   double xs,  double ys)
						: orig{o},                                                  xlenght{ln}, xscale{xs},  yscale{ys}
	{
		if ( x.size() != y.size() )		error("the number of values on the x-axis does not match the number of values on the y-axis");
	...
	Bar_chart::Bar_chart (Point o, const vector< pair<double,double> >& xy, double ln,   double xs,  double ys)
						: orig{o},                 vals(xy),               xlenght{ln}, xscale{xs},  yscale{ys}
	{
		chart_points();
	}
	...
	//Сортировка вектора с парами по значению X
	void Bar_chart::vdpairs_sort(vector< pair<double,double> >& vdp) {
		vector< pair<double,double> > xy;
		
		double xmin {vdp[0].first};
		for (auto p : vdp)
		...
		{
			for (auto e : vdp)
				if ( e.first == xmin )		xy.push_back(e);
			
			//поиск следующего минимального
			for (auto e : vdp) {
				bool lowest {true};		//Является наименьшим из всех элементов, что больше текущего мининимума
				
				for (int i=0; i < vdp.size(); ++i)
					if ( e.first <= xmin || e.first > vdp[i].first )	{ lowest = false;	break; }
				
				if (lowest)		{ xmin = e.first;	break; }
			}
		}
	...
	void Bar_chart::chart_points()
	{
		if ( recs.size() != 0 )		recs.~Vector_ref();		//Удаляем ранее добавленные столбцы гистограммы
		
		if (xlenght < 0)		error("bad Bar_chart lenght");
		
		double dist {(2*xscale) / 3};				//Ширина столбца
		double x {orig.x + xscale - xscale/3};		//Верхний левый угол прямоугольника столбца
		
		if ( isnan(vals[0].first) ) {
			dist = (xlenght / vals.size()) / 2;
			x = orig.x + dist;
		}
		else	sort(vals);
		...
		for (int i=0; i < vals.size(); ++i) {
			if ( isnan(vals[0].first) ) {
				recs.push_back(new Rectangle {Point{int(x), orig.y - int(vals[i].second * yscale)},   int(dist),   int(vals[i].second * yscale)});
				x += dist*2;
			}
	...
	//Число в строку без лишних знаков (например, to_string(double(170)) даёт строку "170,00000000")
	template <typename T>  string to_string_prec(const T val)
	{
		ostringstream oss;
		//oss.precision(n);
		oss << defaultfloat << val;
		return oss.str();
	}
	
	void Bar_chart_txt::chart_points()
	{
		if ( recs.size() != 0 )		marks.~Vector_ref();		//Удаляем ранее добавленные столбцы гистограммы
		
		for (int i=0; i < recs.size(); ++i)
		{
			//Верхняя метка
			if ( isnan(vals[0].first) )
				marks.push_back(new Text {Point{recs[i].point(0).x,   recs[i].point(0).y+recs[i].height()+15}, ""});
			else
				marks.push_back(new Text {Point{recs[i].point(0).x,   recs[i].point(0).y+recs[i].height()+15}, to_string_prec(vals[i].first)});
			
			//Нижняя метка
			marks.push_back(new Text {Point{recs[i].point(0).x,   recs[i].point(0).y-5}, to_string_prec(vals[i].second)});
			
			marks[marks.size()-2].set_color( color() );
			marks[marks.size()-1].set_color( color() );
		}
	}
	...
	Text& Bar_chart_txt::mark(int i, char ud) {
		if (ud != 'u'  &&  ud != 'd')	error("you need to specify the correct direction (up 'u' or down 'd')");
		if (i > col_count()-1)			error("bad Vector_ref range " + to_string(i));
		
		//Верхняя либо нижняя метка
		if (ud == 'u')	return marks[i*2+1];
		else			return marks[i*2];
	}
	
	//------------------------------------------------------------------------------
	
	void String_controller::set_level(int l) {
		if (l < 0)		error("negative level value");
		level = l;
	}
	
	void String_controller::show() const {
		string s = (state) ? "on" : "off";
		cout << "String_controller state: " << s << '\n';
		cout << "Current level: " << level << '\n';
	}
	
	
	
	void Line_color_controller::set_level(int l) {
		if (l < 0)		error("negative level value");
		if (l > 255)	error("level value must be in the range [0; 255]");
		level = l;
		
		if (!state)		return;	//Контроллер выключен
		
		sh.set_color( level );
	}
	
	void Line_color_controller::show() const {
		string s = (state) ? "on" : "off";
		cout << "Line_color_controller state: " << s << '\n';
		cout << "Current (line color) level: " << level << '\n';
	}
	
	
	
	void Line_transparency_controller::set_level(int l) {
		if (l < 0)		error("negative level value");
		if (l > 255)	error("level value must be in the range [0; 255]");
		if (l > 0)		l = 255;
		level = l;
		...
		Color col {sh.color()};
		if (level == 0)		col.set_visibility(Color::invisible);
		else				col.set_visibility(Color::visible);
		sh.set_color( col );
	}
	
	void Line_transparency_controller::show() const {
		string s = (state) ? "on (visible)" : "off (invisible)";
		cout << "Line_transparency_controller state: " << s << '\n';
		cout << "Current (line transparency) level: " << level << '\n';
	}
	
	
	
	void Line_thickness_controller::set_level(int l) {
		if (l < 0)		error("negative level value");
		if (l > 255)	error("level value must be in the range [0; 255]");
		level = l;
		
		Line_style sty { Line_style::Line_style_type( sh.style().style() ),   level };
		sh.set_style( sty );
	}
	
	void Line_thickness_controller::show() const {
		string s = (state) ? "on" : "off";
		cout << "Line_thickness_controller state: " << s << '\n';
		cout << "Current (line thickness) level: " << level << '\n';
	}
	...
	void print(Iterator& iter) {
		...
		for (double* num = iter.next();   num != nullptr;   num = iter.next())
		{
			++count;
			cout << *num << '\n';	//разыменование указателя (получение значения по указанному адресу памяти)
		}
		
		if (count == 0)		cout << "empty object" << '\n';
	}
	...
	//Заполнение группы mrks узлами в виде фигур Circle
	void Binary_tree::grow() {
		...
		mrks.add_shape(new Circle {root_c, r});
		...
		{
			cx /= 2;
			int start_x = mrks[ mrks.size() - pow(2,i-1) ].point(0).x+r - cx/2;	//левее центра первого слева кружка на пред. уровне
			
			for (int j=0; j < pow(2,i); ++j)			//кол-во кружков на уровне с поряд.номером i+1
			{
				Point cntr {start_x + cx*j,   root_c.y + (8*r)*i};
				mrks.add_shape(new Circle {cntr, r});
			}
		}
		
		for (unsigned int i=0; i<mrks.size(); ++i)
			txt.push_back( new Text {Point {mrks[i].point(0).x+r+r/2, mrks[i].point(0).y-2}, ""} );
	}
	...
	void Binary_tree::draw_lines() const {
		...
		if (fill_color().visibility()) {
			...
			for (unsigned int i=1; i<mrks.size(); ++i)
				fl_pie(mrks[i].point(0).x, mrks[i].point(0).y,  r+r-1, r+r-1,  0, 360);
		...
		if (color().visibility()) {
			...
			for (int i=1; i < levels; ++i) {				//степени двойки
				...
				for (int j=0; j < pow(2,i); j+=2)			//кол-во кружков на уровне с поряд.номером i+1
				{
					...
					Point tmp_cntr {mrks[N + j/2 - pow(2,i-1)].point(0).x+r, mrks[N + j/2 - pow(2,i-1)].point(0).y+r};
					Circle tmp {tmp_cntr, r};
					
					for (int k=0; k < 2; ++k) {				//пары   кружков
						//Текущая перебираемая метка
						Point curr_cntr {mrks[N+j+k].point(0).x+r, mrks[N+j+k].point(0).y+r};
						Circle curr {curr_cntr, r};
						
						if ( direction.size() == 0 )
							l.add_shape(new Line  { s(tmp),   n(curr) });
						else if ( direction == up )
							l.add_shape(new Arrow { n(curr),   s(tmp) });
						else if ( direction == down )
							l.add_shape(new Arrow { s(tmp),   n(curr) });
			...
			for (unsigned int i=0; i<mrks.size(); ++i) {
				Circle shape_c {Point {mrks[i].point(0).x+r, mrks[i].point(0).y+r}, r};
				shape_c.set_color( color().as_int() );
				shape_c.set_style( style() );
				shape_c.draw();
			}
			
			//Обеспечивает отображение непустого текста рядом с метками
			for (unsigned int i=0; i<txt.size(); ++i)
				if (txt[i].label().size() > 0) {
					Text t {txt[i].point(0), txt[i].label()};
					t.set_color( color().as_int() );
					t.draw();
				}
		}
	}
	
	//Установить значение надписи рядом с меткой, путь от корня к которой указан в 1ом аргументе
	void Binary_tree::set_mark_label(string d, string lbl) {
		if (d.size() > levels)
			error("the specified route to the node is outside the tree (route size greater that tree levels count)");
		...
		for (int i=1; i < d.size(); ++i)
			if (d[i] == 'l')		id = 2*id + 1;
			else if (d[i] == 'r')	id = 2*id + 2;
			else					error("illegal route symbol (" + string(1, d[i]) + ") in position " + to_string(i));
		
		txt[id].set_label(lbl);
		
		Shape::set_color( color().as_int() );
	}
	...
	//Заполнение группы mrks узлами в виде фигур TXTBox
	void Binary_txt_tree::grow() {
		...
		mrks.add_shape(new TXTBox {point(0), w, h, r, "1"});
		
		int tx = w + w/4;		//Расстояние между верхними левыми углами TXTBox'ов
		...
		{
			tx /= 2;
			int start_x = mrks[ mrks.size() - pow(2,i-1) ].point(0).x - tx/2; //левее первого слева TXTBox'а на пред. уровне
			
			for (int j=0; j < pow(2,i); ++j)			//кол-во TXTBox'ов на уровне с поряд.номером i+1
			{
				Point new_p {start_x + tx*j,   point(0).y + (8*r)*i};
				mrks.add_shape(new TXTBox {new_p, w, h, r, to_string(mrks.size()+1)});
			}
		}
		
		for (unsigned int i=0; i<mrks.size(); ++i)
			txt.push_back( new Text {Point {mrks[i].point(0).x+w/2+2, mrks[i].point(0).y-2}, ""} );
	}
	...
	void Binary_txt_tree::draw_lines() const {
		...
		if (fill_color().visibility())
			for (unsigned int i=0; i<mrks.size(); ++i) {
				TXTBox shape_t {mrks[i].point(0), w, h, r, to_string("\n")};	//пустой текст
		...
		if (color().visibility()) {
			...
			for (int i=1; i < levels; ++i) {				//степени двойки
				...
				{
					//Родительсая метка для следующей пары меток
					Point tmp_lup {mrks[N + j/2 - pow(2,i-1)].point(0).x, mrks[N + j/2 - pow(2,i-1)].point(0).y};
					TXTBox tmp {tmp_lup, w, h, r, "\n"};
					
					for (int k=0; k < 2; ++k) {				//пары   кружков
						//Текущая перебираемая метка
						Point curr_lup {mrks[N+j+k].point(0).x, mrks[N+j+k].point(0).y};
						TXTBox curr {curr_lup, w, h, r, "\n"};
			...
			//Обеспечивает работу ф-ций Shape::set_color()  И  Shape::set_style()
			for (unsigned int i=0; i<mrks.size(); ++i) {
				TXTBox shape_t {mrks[i].point(0), w, h, r, to_string(i+1)};
	...
	bool Poly::check_intersections()
	{
		for (int i = 0; i < fig.size(); ++i)
			for (int j = i+2; j < fig.size(); ++j)
			{
				...
				if ( j == fig.size()-1  &&  i!=0 )
					is = intersect(  fig[i], fig[i+1], fig[j],  fig[0]   );
				else if ( j != fig.size()-1 )
					is = intersect(  fig[i], fig[i+1], fig[j], fig[j+1]  );
	...
	void Regular_polygon::angles()
	{
		for (int i = 0; i <= n; ++i) {
			...
			if (n == 3)		angle += (M_PI / 2);	//90 градусов
			if (n == 4)		angle += (M_PI / 4);	//45 градусов
			if (n == 5)		angle += (M_PI / 2);	//90 градусов
	...
	void TXTBox::draw_lines() const
	{
		...
		fl_draw(lbl.c_str(), point(0).x+5, point(0).y+17);
		fl_font(ofnt,osz);
	}
	
	//------------------------------------------------------------------------------
	
	void Scheme_tree::draw_lines() const {
		//Сначала рисуем соединяющие линии
		if (childs.size() > 0) {
			int free_space = childs[0].point(0).x - (point(0).x + width());
			
			//Линия справа, от родительского узла
			Point p0 { e(*this) };
			fl_line( p0.x, p0.y,   p0.x+free_space/2, p0.y );
			
			//Вертикальная общая линия, соединяющая все остальные
			Point p1 { Graph_lib::w(childs[0]) };
			if (p1.y > p0.y)	p1.y = p0.y;
			
			Point p2 { Graph_lib::w(childs[childs.size()-1]) };
			if (p2.y < p0.y)	p2.y = p0.y;
			
			fl_line( p1.x-free_space/2, p1.y,   p2.x-free_space/2, p2.y );
			
			//Линии слева, от дочерних ветвей
			for (int i=0; i<childs.size(); ++i) {
				Point tmp { Graph_lib::w(childs[i]) };
				fl_line( tmp.x-free_space/2, tmp.y,   tmp.x, tmp.y );
			}
		}
		
		//Рисуем корень текущего дерева/начала ветви
		TXTBox::draw_lines();
		
		//Дочерние рисуются при вызове вложенного draw_lines() в каждый объект (и в объектах вложенных в них так же)
		for (int i=0; i<childs.size(); ++i)
			childs[i].draw_lines();
	}
	
	void Scheme_tree::add_childs(Point p, int ww, const vector<string>& s)
	{
		for (int i=0; i<s.size(); ++i)
		{
			//перебирается не первый элемент
			if (i > 0)
				p.y += height() + y_inc;
			
			childs.push_back(new Scheme_tree {p, ww, height(), s[i]});
			childs[childs.size()-1].set_color( color() );
			childs[childs.size()-1].set_fill_color( fill_color() );
			childs[childs.size()-1].set_style( style() );
			childs[childs.size()-1].set_font( font() );
			childs[childs.size()-1].set_font_size( font_size() );
