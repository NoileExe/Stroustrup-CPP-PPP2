...
public:
	...
	void erase(int v);		//Удаляет список со значением v меняя все ссылки
	
	//Указатель на верхний элемент (макс. уровень) заголовочного списка (val = 0)
	const Unit* header() const { return vu[0]; }
	int level() const { return vu[0]->lvl; }

private:
	vector<Unit*> vu;	//Верхушки списков (крайний верхний элемент)
	
	void ident_succ();	//Идентификация наследников (указание элементов справа от каждого в списках на каждом уровне)
};
