...
namespace addition_intermediate_version {
	...
	Binary_tree::Binary_tree(Point p, int lvl) : levels{lvl} {
		if (lvl<0)		error("negative levels value");
		add(p);
		grow();
	}
	
	Binary_tree::Binary_tree(Point p, int lvl, string str) : levels{lvl}, direction{str} {
		if (lvl<0)		error("negative levels value");
		if ( str != up  &&  str != down )
	...
	Binary_tree::Binary_tree(Point p, int lvl, string str, Color col) : levels{lvl}, direction{str} {
		if (lvl<0)		error("negative levels value");
		if ( str != up  &&  str != down )
			error("bad value of Binary_tree arrow direction(" + str +")");
		lc = col;
	...
	void Binary_tree::draw_lines() const {
		...
		for (unsigned int i=0; i < c.size(); ++i)    c[i].draw();
	}
	...
	void Binary_tree::grow() {
		...
		c.push_back(new Circle {point(0), r});
		c[c.size()-1].set_color( color().as_int() );
		c[c.size()-1].set_style( Line_style(Line_style::solid, 2) );
		...
		{
			cx /= 2;
			int start_x = c[ c.size() - pow(2,i-1) ].center().x - cx/2; //левее первого слева кружка на пред. уровне
			
			for (int j=0; j < pow(2,i); j+=2)			//кол-во кружков на уровне с поряд.номером i+1
			{
				//Родительский круг для следующей пары кружков
				Circle& pc = c[ c.size() - pow(2,i-1) - j/2 ];
				
				//Вариант расстановки отталкиваясь от точки левее центра кружка пред. уровня
				for (int k=0; k < 2; ++k) {				//пары   кружков
					Point cntr {start_x + cx*(j+k),   point(0).y + (8*r)*i};
					...
					if ( direction.size() == 0 ) {
						l.add_shape(new Line { s(pc),   n(c[c.size()-1]) });
						l[l.size()-1].set_style( Line_style(Line_style::solid, 2) );
					}
					else if ( direction == up )
						l.add_shape(new Arrow { n(c[c.size()-1]),   s(pc) });
					else if ( direction == down )
						l.add_shape(new Arrow { s(pc),   n(c[c.size()-1]) });
					
					l[l.size()-1].set_color( lc );
				}
				
				//Вариант расстановки ниже слева и справа от центра каждого родительского кружка
				/*for (int k=0; k < 2; ++k) {				//пары   кружков
					Point cntr {pc.point(0).x+r, point(0).y + (8*r)*i};
					...
					l[l.size()-1].set_color( color().as_int() );
				}*/
			}
		}
	}
	...
	void Binary_tree::set_color(Color col) {
		...
		for (int i=0; i<c.size(); ++i)		c[i].set_color(col);
	}
	...
	void Binary_tree::move(int dx, int dy) {
		...
		for (int i=0; i<c.size(); ++i)		c[i].move(dx,dy);
	}
	...
	void Binary_txt_tree::grow() {
		...
		int w {35};
		int h {25};
		Point p {point(0).x-w/2, point(0).y-h/2};
		
		//Самый верхний TXTBox (корень) дерева
		t.push_back(new TXTBox {p, w, h, 5, "1"});
		t[t.size()-1].set_color( color().as_int() );
		t[t.size()-1].set_style( Line_style(Line_style::solid, 2) );
		...
		for (int i=1; i < levels; ++i)					//степени двойки
		{
			...
			for (int j=0; j < pow(2,i); j+=2)			//кол-во TXTBox'ов на уровне с поряд.номером i+1
			{
				//Родительский TXTBox для следующей пары TXTBox'ов
				TXTBox& pt = t[ t.size() - pow(2,i-1) - j/2 ];
				
				for (int k=0; k < 2; ++k) {				//пары   кружков
					Point new_p {start_x + tx*(j+k),   p.y + (8*r)*i};
					...
					if ( direction.size() == 0 )
						l.add_shape(new Line { s(pt),   n(t[t.size()-1]) });
					else if ( direction == up )
						l.add_shape(new Arrow { n(t[t.size()-1]),   s(pt) });
					else if ( direction == down )
						l.add_shape(new Arrow { s(pt),   n(t[t.size()-1]) });
