...
int main()
{
	while (true) {
		try
		{
			Point tl{win_centering( x_max(), y_max() )}; //Расположение ровно по середине экрана
			...
			for (int i = 0; i < 9; ++i) {
				...
				Binary_tree bt {Point{cntr.x, 40}, i};
			...
			for (int i = 0; i < 9; ++i) {
				...
				Binary_txt_tree bt {Point{cntr.x, 40}, i};
			...
			Binary_tree btu {Point{cntr.x, 40}, 6, "up"};
			
			win.attach( btu );
			win.wait_for_button();
			
			btu.set_mark_label("l", "root");
			win.wait_for_button();
			
			btu.set_mark_label("r", "Root");
			win.wait_for_button();
			
			btu.set_mark_label("lr", "lr");
			win.wait_for_button();
			
			btu.set_color(Color::dark_red);
			win.wait_for_button();
			
			btu.set_mark_label("lrlrr", "lrlrr");
			win.wait_for_button();
			...
			Binary_tree btd {Point{cntr.x, 40}, 7, "down", Color::magenta};
			//Binary_txt_tree btd {Point{cntr.x, 40}, 5, "down", Color::magenta};
			win.attach( btd );
			win.wait_for_button();
			...
			Binary_txt_tree bttd {Point{cntr.x, 40}, 5, "down", Color::magenta};
			win.attach( bttd );
			win.wait_for_button();
			
			bttd.set_fill_color(Color::green);
			win.wait_for_button();
			
			bttd.set_mark_label("l", "root");
			win.wait_for_button();
			
			bttd.set_mark_label("r", "Root");
			win.wait_for_button();
			
			bttd.set_mark_label("lr", "lr");
			win.wait_for_button();
			
			bttd.set_color(Color::dark_red);
			win.wait_for_button();
			
			bttd.set_mark_label("lrlrr", "lrlrr");
			win.wait_for_button();
			
			bttd.set_arrow_color(Color::cyan);
			win.wait_for_button();
