...
template<class T>
T my_dot_product(const Numeric_lib::Matrix<T>& a, const Numeric_lib::Matrix<T>& b)
{
	if(a.size() != b.size())		error("different sizes of matrices [ my_dot_product(Matrix a, Matrix b) ]");
	T res = 0;
...
template<class T>
Numeric_lib::Matrix<T> my_scale_and_add(const Numeric_lib::Matrix<T>& arg1, T arg2, const Numeric_lib::Matrix<T>& arg3)
{
	if(arg1.size() != arg3.size())		error("different sizes of matrices [ my_scale_and_add(Matrix arg1, arg2, Matrix arg3) ]");
	Numeric_lib::Matrix<T> res (arg1.size());
	for(Index i = 0; i < arg1.size(); ++i)		res[i] = arg1[i] * arg2 + arg3[i];
	return res;
}
...
void classical_elimination(Matrix& A, Vector& b)
{
	...
	for (Index j = 0; j < n-1; ++j)
	{
		...
		for (Index i = j+1; i < n; ++i)
		{
			...
			A[i].slice(j) = my_scale_and_add(A[j].slice(j), -mult, A[i].slice(j));
